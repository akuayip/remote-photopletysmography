...
class RespirasiProcessor:
    ...
    def extract_resp_from_frame(self, frame):
        ...
        if results.pose_landmarks:
            ...
            # Ambil titik bahu kiri dan kanan (id 11 dan 12)
        ...
        if len(self.shoulder_motion_signal) >= 30:
            self.filtered_signal = bandpass_filter_respirasi(self.shoulder_motion_signal, fs=self.fps)
            self.respiration_rate, _ = calculate_respiration_rate(self.filtered_signal, fs=self.fps)
        else:
